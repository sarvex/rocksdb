...
class IniParser:
    ...
    def remove_trailing_comment(line):
        ...
        return line[:comment_start] if comment_start > -1 else line
    ...
    def is_section_header(line):
        ...
        line = line.strip()
        return bool(line.startswith("[") and line.endswith("]"))
    ...
    def get_section_name(line):
        ...
        if len(token_list) < 3:
            ...
            raise ValueError(f"Parsing error: {error}" + "\n" + line)
        return token_list[1]
    ...
    def get_key_value_pair(line):
        ...
        if not value:  # if the option has no value
            return (key, None)
        values = IniParser.get_list_from_value(value)
        return (key, value) if len(values) == 1 else (key, values)
